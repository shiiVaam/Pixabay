...
AF.request("\(Constants.Defaults.BASE_URL)key=\(Constants.Defaults.pixApiKey)&q=\(finalText)&image_type=photo&page=\(pageNumber)&per_page=20",method: .get).responseJSON { (response) in
...
extension HomeVC:UITextFieldDelegate{
    ...
    func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        ...
        if textField.text?.count ?? 0 > 2{
            CoreDataManager.shared.fetchDataFromCoreData(searchedValue: textField.text!) { [weak self](result) in
